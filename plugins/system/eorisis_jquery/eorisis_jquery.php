...
class plgSystemEorisis_jQuery extends JPlugin
{
	...
	{
		require_once(JPATH_SITE.'/plugins/system/eorisis_jquery/framework/lib/eorisis_jquery.php');
			...
			($migrate == 1) and version_compare($lib_version, '1.9.0', '>=')))
		{
			...
			if ($this->params->get('migrate_source', 1) == 1)
			{
				...
				switch ($this->params->get('migrate_source_cdn', 1))
				{
					...
					case 3: $url = $this->cdn->microsoft.'jquery.migrate/jquery-migrate-'.$migrate_version.'.min.js'; break;
					default: $url = $default;
				}
		...
		if (!$this->params->get('ui_custom', 0))
		{
			...
			if ($this->params->get('ui_source', 1) == 1)
			{
				...
				switch ($this->params->get('ui_source_cdn', 1))
				{
					...
					case 3: $main_url = $this->cdn->microsoft.'jquery.ui/'; break;
	...
	protected function no_conflict()
	{
		...
		if (!$nc or (($nc == 1) and !$this->url->jq_lib)) { return; }
	...
	protected function cdn_fallback($field, $url, $url_local)
	{
		...
		{
			if (function_exists('curl_init')) // or curl_version
			{
	...
	protected function set_urls()
	{
		...
		$this->cdn = (object)array(
			...
			'microsoft'			=> '//ajax.aspnetcdn.com/ajax/'); // Microsoft
	...
	protected function remove_media_strings($type, $arr, $strings)
	{
		if ($strings)
		{
			...
			foreach ($urls as $url => $attr)
			{
				...
				{
					if ((strpos($url, $string) !== false) and
						!in_array($url, $this->loaded_media))
					{
						$arr[] = $url;
					}
				}
