...
// Set default panel
if ($panel == '' && in_array($widget->position, array('slideshow', 'top-a', 'top-b', 'bottom-a', 'bottom-b', 'main-top', 'main-bottom', 'sidebar-a', 'sidebar-b'))) {
    $panel = $this['config']->get("panel_default.{$widget->position}.panel", '');
}
// Set panel for specific positions
else if (in_array($widget->position, array('headerbar', 'toolbar-r' ,'toolbar-l', 'footer', 'offcanvas', 'sidepanel'))) {
	$panel = 'uk-panel';
}
...
if (array_key_exists('animation', $params)) {
		$animation = 'data-uk-scrollspy="{cls:\''.$params['animation'].'\'}"';
	} else{
		$animation = "";
	} 
